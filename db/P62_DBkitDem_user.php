...
function user_delete($username) {
    ...
    $queryStr = 'DELETE FROM ' . P62_DBKITDEM_TB_USER. ' WHERE ' . get_tb_col_pair(USER_TB_COL_USERNAME);
    try {
...
function user_authenticate($username, $password) {
	global $pdo;
	$resultat = false; // Mode défensif
    $queryStr = 'SELECT * FROM ' . P62_DBKITDEM_TB_USER. ' WHERE ' . get_tb_col_pair(USER_TB_COL_USERNAME);
    try {
        $sth = $pdo->prepare($queryStr);
        $params = array(
            COLON_CAR . USER_TB_COL_USERNAME => $username,
...
function user_add($username, $password, $firstname, $lastname, $email) {
    ...
    $resultat = false; // Mode défensif
    $queryStr = 'INSERT INTO ' . P62_DBKITDEM_TB_USER . '(' . get_tb_cols($user_tb_cols) . ') VALUES (' . get_tb_cols($user_tb_cols, COLON_CAR) . ')';
    $sth = $pdo->prepare($queryStr);
...
function user_log_in($user_id, $session_id) {
    ...
    $resultat = false;
    $queryStr = 'INSERT INTO ' . P62_DBKITDEM_TB_USERCNX . ' (' . get_tb_cols($usercnx_tb_cols) . ') VALUES (' . get_tb_cols($usercnx_tb_cols, COLON_CAR) . ')';
    $sth = $pdo->prepare($queryStr);
    $params = array(
...
function user_log_out($user_id, $session_id) {
    global $pdo;
    $resultat = false;
    $queryStr = 'UPDATE ' . P62_DBKITDEM_TB_USERCNX
...
function user_list($cnx_state = false) {
    ...
    $queryStr = 'SELECT u.' . USER_TB_COL_USERNAME
        . ' FROM ' . P62_DBKITDEM_TB_USER. ' AS u'
        . ' JOIN ' . P62_DBKITDEM_TB_USERCNX . ' AS uc ON (u.id = uc.user_id)'
